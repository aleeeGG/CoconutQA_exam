...
class CustomRequester:
    ...
    def send_request(self, method, endpoint, data=None, params=None, expected_status=200, need_logging=True, **kwargs):
        ...
        if response.status_code != expected_status:
            raise ValueError(
                f"Пришел: {response.status_code}. Ожидался: {expected_status}"
            )
    ...
    def log_request_and_response(self, response):
        try:
            ...
            if not is_success:
                self.logger.info(
                    f"\tКОД СТАТУСА: {RED}{response_status}{RESET}\n"
                    f"\tДАННЫЕ: {RED}{response_data}{RESET}"
                )
            else:
                self.logger.info(
                    f"\tКОД СТАТУСА: {GREEN}{response_status}{RESET}\n"
                    f"\tДАННЫЕ:\n{response_data}"
                )
            self.logger.info(f"{'=' * 80}\n")
        except Exception as e:
            self.logger.error(f"\nОшибка логирования: {type(e)} - {e}")